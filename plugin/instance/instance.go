...
func (h *instanceHandler) attachTo(router *mux.Router) {
	router.HandleFunc("/", server.OutputHandler(h.describe)).Methods("GET")
	router.HandleFunc("/", server.OutputHandler(h.provision)).Methods("POST")
	router.HandleFunc("/{key}", server.OutputHandler(h.destroy)).Methods("DELETE")
}
