...
import (
	...
	"github.com/docker/libmachete/plugin/flavor/zookeeper"
...
func main() {
	...
	pluginLookup := func(key string) (instance.Plugin, error) {
		switch key {
		case "vagrant":
			return vagrant.NewVagrantPlugin("/Users/billfarner/Desktop/tmp"), nil
		default:
			return nil, errors.New("Unknown instance plugin")
		}
	}

	flavorLookup := func(key string) (flavor.Plugin, error) {
		switch key {
		case "zookeeper":
			return zookeeper.NewPlugin(), nil
		default:
			return nil, errors.New("Unknown instance plugin")
		}
	}
...
Once the server is running, other subcommands may be used as a convenience to interact with groups`,
		Run: func(cmd *cobra.Command, args []string) {
			groupserver.Run(port, pluginLookup, flavorLookup)
		},
	}
