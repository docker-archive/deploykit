...
var (
	...
	provisionHelper = testProvisionHelper{tags: map[string]string{"custom": "value"}}
)
...
func mockedPluginGroup(ctrl *gomock.Controller) (*mock_instance.MockPlugin, group.Plugin) {
	...
	grp := NewGroupPlugin(map[string]instance.Plugin{pluginName: plugin}, &provisionHelper, 1*time.Millisecond)
	return plugin, grp
}
...
func provisionTags(config group.Configuration) map[string]string {
	...
	for k, v := range provisionHelper.tags {
		tags[k] = v
	}
