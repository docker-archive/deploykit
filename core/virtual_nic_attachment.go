...
func (v *VNicClient) GetVNicAttachment(vNicAttachmentID string) VNicAttachment {
	vNicAttachment := VNicAttachment{}
	queryString := url.QueryEscape(vNicAttachmentID)
	resp, err := v.Client.Get("/vnicAttachments/" + queryString)
	if err != nil {
		logrus.Error(err)
	}
...
func (v *VNicClient) ListVNicAttachments(instanceID string) []VNicAttachment {
	vNicAttachments := []VNicAttachment{}
	queryString := url.QueryEscape(vc.Client.compartmendID)
	if instanceID != "" {
		queryString = queryString + "&" + url.QueryEscape(instanceID)
	}
	resp, err := v.Client.Get(fmt.Sprintf("/vnicAttachments?compartmentId=%s", queryString))
	if err != nil {
		logrus.Error(err)
	}
	logrus.Debug("StatusCode: ", resp.StatusCode)
